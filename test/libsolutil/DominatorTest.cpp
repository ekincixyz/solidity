...
public:
	DominatorFinderTest(
		...
		std::vector<TestVertexLabel> const& _expectedIdom,
	...
	std::vector<TestVertexId> labelToId(std::vector<TestVertexLabel> const& _vertexLabels) const
	{
		return _vertexLabels
			| ranges::views::transform([&](TestVertexLabel const& label) {
				soltestAssert(verticesIdMap.count(label) > 0);
				return verticesIdMap.at(label);
			})
			| ranges::to<std::vector<TestVertexId>>;
	}

	// Converts a map of vertex label to DFS indices to a vector of DFS indices.
	// The vector is indexed by vertex Id.
	// NOTE: Only used for testing purposes.
	std::vector<size_t> labelMapToVertexId(std::map<TestVertexLabel, size_t> const& _dfsIndices) const
	{
		std::vector<size_t> indicesById(vertices.size());

		for (auto const& [label, dfsIndex] : _dfsIndices)
		{
			soltestAssert(verticesIdMap.count(label) > 0);
			indicesById[verticesIdMap.at(label)] = dfsIndex;
		}
	...
	// Converts a map of vertex label to set of labels back to a Dominator tree, i.e. vector of vertex IDs to sets of vertex IDs
	// NOTE: Only used for testing purposes.
	std::vector<std::set<TestVertexId>> dominatorTreeByVertexId(std::map<TestVertexLabel, std::set<TestVertexLabel>> const& _domTreeByLabel) const
	{
		...
		std::vector<std::set<TestVertexId>> domTree(vertices.size());
	...
	std::vector<TestVertexId> expectedIdom;
	std::vector<size_t> expectedDFSIndices;
	std::vector<std::set<TestVertexId>> expectedDominatorTree;
};
...
BOOST_AUTO_TEST_CASE(immediate_dominator_1)
{
	...
	DominatorFinderTest test(
		...
		{"A", "A", "B", "B", "D", "B", "C", "G"}, // Immediate dominators
		{ // DFS indices
			{"A", 0},
			{"B", 1},
			{"C", 2},
...
BOOST_AUTO_TEST_CASE(itworst)
{
	...
	DominatorFinderTest test(
		...
		{"R", "R", "R", "R", "R", "X1", "X2", "X3", "Y1", "Y2", "R", "R", "R"},
		{
			{"R", 0},
			{"W1", 1},
			{"W2", 2},
