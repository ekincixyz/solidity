...
public:
	...
	DominatorFinder(V const& _entry, size_t _numVertices):
		...
		m_dfsIndexByVertexId(_numVertices),
		m_dominatorTree(_numVertices),
	...
	std::vector<DfsIndex> const& dfsIndexById() const
	{
		return m_dfsIndexByVertexId;
	}

	std::vector<VId> const idom() const
	{
		auto toId = [&](DfsIndex const& _vIdx) { return m_verticesInDFSOrder[_vIdx]; };
		return m_immediateDominators | ranges::views::transform(toId) | ranges::to<std::vector<VId>>;
	}
	...
	std::vector<std::set<VId>> const& dominatorTree() const
	{
	...
	bool dominates(VId const& _aId, VId const& _bId) const
	{
		...
		solAssert(_aId >= 0 && _aId < m_dfsIndexByVertexId.size());
		solAssert(_bId >= 0 && _bId < m_dfsIndexByVertexId.size());
	...
	std::vector<VId> dominatorsOf(VId const& _vId) const
	{
		...
		solAssert(_vId >= 0 && _vId < m_dfsIndexByVertexId.size());
...
private:
	std::vector<DfsIndex> findDominators(V const& _entry, size_t _numVertices)
	{
		...
		solAssert(_entry.id == 0, "Entry vertex ID must be zero");
		...
		auto dfs = [&](V const& _v, auto _dfs) -> void {
			solAssert(_v.id >= 0 && _v.id < _numVertices);
			...
			ForEachSuccessor{}(_v, [&](V const& w) {
				solAssert(w.id >= 0 && w.id < _numVertices);
	...
	void buildDominatorTree()
	{
		...
		for (DfsIndex index = 1; index < m_verticesInDFSOrder.size(); ++index)
		{
			...
			solAssert(index == m_dfsIndexByVertexId[vId]);
	...
	/// Ids are assumed to be a contiguous sequence starting from 0 to the number of vertices in the graph.
	///
	/// Vertex id -> DFS index
	std::vector<DfsIndex> m_dfsIndexByVertexId;

	/// Maps a Vertex to all vertices that it dominates.
	/// The value in the array is a set of vertices Ids that are dominated by the vertex which id is the array index.
	///
	/// Vertex id -> dominates set {Vertex id}
	std::vector<std::set<VId>> m_dominatorTree;
